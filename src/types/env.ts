 ...
 * - X-Loom-Base-URL: (Optional) Override the default Loom API base URL
 */
...
export interface TenantCredentials {
  /** OAuth Access Token for Loom API (from X-Loom-Access-Token header) */
  accessToken?: string;

  /** Override Loom API base URL (from X-Loom-Base-URL header) */
  baseUrl?: string;
}
...
export function parseTenantCredentials(request: Request): TenantCredentials {
  ...
  return {
    accessToken: headers.get('X-Loom-Access-Token') || undefined,
    baseUrl: headers.get('X-Loom-Base-URL') || undefined,
  };
}
...
export function validateCredentials(credentials: TenantCredentials): void {
  if (!credentials.accessToken) {
    throw new Error('Missing credentials. Provide X-Loom-Access-Token header.');
  }
}
